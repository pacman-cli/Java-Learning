...
public class BillingServiceImpl implements BillingService {
    ...
    public BillingServiceImpl(BillingMapper billingMapper, BillingRepository billingRepository, PatientRepository patientRepository, AppointmentRepository appointmentRepository, InsuranceRepository insuranceRepository) {
...
//    }
    ...
    public BillingDto createBilling(BillingDto billingDto) {
        ...
        var patient = patientRepository.findById(billingDto.getPatientId()).orElseThrow(() -> new ResourceNotFoundException("Patient not found with id: " + billingDto.getPatientId()));
        ...
        if (billingDto.getAppointmentId() != null) {
            var appointment = appointmentRepository.findById(billingDto.getAppointmentId()).orElseThrow(() -> new ResourceNotFoundException("Appointment not found with id: " + billingDto.getAppointmentId()));
            billing.setAppointment(appointment);
        }

        //billing date defaults to current date
        ...
        if (billingDto.getInsuranceId() != null) {
            insurance = insuranceRepository.findById(billingDto.getInsuranceId())
                    .orElseThrow(() -> new ResourceNotFoundException("Insurance not found: " + billingDto.getInsuranceId()));
        } else {
            // try to find an active insurance for the patient on billing date
//            List<Insurance> insList = insuranceRepository.findAllByPatientId(patient.getId());
//            var billingDateLocal = billing.getBillingDate().toLocalDate();
//            for (Insurance ins : insList) {
//                if ((ins.getValidFrom() == null || !ins.getValidFrom().isAfter(billingDateLocal))
//                        && (ins.getValidTo() == null || !ins.getValidTo().isBefore(billingDateLocal))) {
//                    insurance = ins; //if valid, then set insurance
//                    break;
//                    // Start date validation: ins.getValidFrom() == null || !ins.getValidFrom().isAfter(billingDateLocal)
//                    //If validFrom is null → insurance has no start restriction (always valid from the beginning)
//                    //If validFrom is not after billing date → insurance has already started
//                    //End date validation: ins.getValidTo() == null || !ins.getValidTo().isBefore(billingDateLocal)
...
//            }
            ...
            List<Insurance> inList = insuranceRepository.findByPatientIdAndValidFromLessThanEqualAndValidToGreaterThanEqual(patient.getId(), billingDateLocal, billingDateLocal);
            Insurance ins = inList.isEmpty() ? null : inList.get(0);
        }
        if (insurance != null) {
            ...
            if (coveragePercent != null) {
                ...
                BigDecimal amount = billing.getAmount() != null ? billing.getAmount() : BigDecimal.ZERO;
                BigDecimal covered = amount.multiply(percentage).divide(BigDecimal.valueOf(100));
                BigDecimal patientPays = amount.subtract(covered);

                //append to description
                String coverLine = String.format("Insurance %s (%s) covers %s%% -> covered: %s, patient pays: %s",
                        insurance.getPolicyNumber(), insurance.getProviderName(), percentage.toPlainString(),
                        covered.setScale(2, RoundingMode.HALF_UP).toPlainString(),
                        patientPays.setScale(2, RoundingMode.HALF_UP).toPlainString());

                String description = billing.getDescription() != null ? billing.getDescription() + "|" + coverLine : coverLine;
                //update description
            ...
            // no insurance found: ensure numeric fields null
            billing.setCoveredAmount(null);
